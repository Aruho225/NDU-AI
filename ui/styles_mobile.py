from ui.brand_colors import NDU_BORDER, NDU_NAVY, NDU_SURFACE, NDU_TEXT, NDU_WHITE

MOBILE_CSS = f"""
@media (max-width: 768px) {{
    .block-container {{ padding-top: 0.75rem; padding-left: 0.75rem; padding-right: 0.75rem; max-width: 100%; }}
    .hero {{ padding: 0.9rem 1rem; border-radius: 4px; }}
    ...
    .mobile-tip {{
        background: {NDU_SURFACE}; border: 1px solid {NDU_BORDER}; border-radius: 4px;
        padding: 0.65rem 0.8rem; margin-bottom: 0.75rem; font-size: 0.88rem; color: {NDU_NAVY};
    }}
